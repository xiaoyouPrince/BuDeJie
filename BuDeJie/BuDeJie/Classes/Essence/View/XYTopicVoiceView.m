...
- (void)setModel:(XYTopic *)model
{
    ...
    UIImage *originImage = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:model.image1];
    if (originImage) { // 原图已经被下载过
        self.imageView.image = originImage;
    } else { // 原图并未下载过
        if (mgr.isReachableViaWiFi) {
            [self.imageView sd_setImageWithURL:[NSURL URLWithString:model.image1] placeholderImage:placeholder];
...
#warning downloadOriginImageWhen3GOr4G配置项的值需要从沙盒里面获取
            ...
            if (downloadOriginImageWhen3GOr4G) {
                ...
                [self.imageView sd_setImageWithURL:[NSURL URLWithString:model.image0] placeholderImage:placeholder];
            }
        } else { // 没有可用网络
            UIImage *thumbnailImage = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:model.image0];
